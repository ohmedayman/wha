...
const AGENT_REQUESTS_FILE = path.join(DATA_DIR, 'agent_requests.json');
...
let memoryAuditLogs = [
    {
        ...
        action: '🚀 SYSTEM_START',
        username: 'SYSTEM',
        details: 'تم تشغيل المنظومة السحابية المركزية وإدارة طلبات الوكلاء بنجاح'
    }
];
...
function getAgentRequests() { return readJsonSafe(AGENT_REQUESTS_FILE, memoryAgentRequests); }
function saveAgentRequests(reqs) { memoryAgentRequests = reqs; writeJsonSafe(AGENT_REQUESTS_FILE, reqs); }
...
module.exports = (req, res) => {
    ...
    try {
        ...
        // ==========================================
        // 👔 AGENT PUBLIC REGISTRATION & APPLICATION
        // ==========================================

        // Public Agent Application
        if (pathname === '/agent/apply' && method === 'POST') {
            const { name, username, password, phone, company, city, expectedSales, notes } = body;

            if (!name || !username || !password || !phone) {
                return sendJson(400, { success: false, error: 'يرجى ملء الحقول الإجبارية (الاسم، اسم المستخدم، كلمة السر، الهاتف)' });
            }

            const cleanUser = username.trim().toLowerCase();
            const agents = getAgents();
            const requests = getAgentRequests();

            if (agents.some(a => a.username.toLowerCase() === cleanUser) || requests.some(r => r.username.toLowerCase() === cleanUser && r.status === 'pending')) {
                return sendJson(400, { success: false, error: 'اسم المستخدم هذا مسجل مسبقاً أو يوجد طلب قيد المراجعة بنفس الاسم' });
            }
            ...
            const newReq = {
                ...
                city: (city || '').trim(),
                expectedSales: (expectedSales || '').trim() || '10-30 ترخيص شهرياً',
                notes: (notes || '').trim(),
                status: 'pending',
                createdAt: new Date().toISOString()
            };
            ...
            saveAgentRequests(requests);
            addAuditLog('📝 AGENT_APPLY', newReq.username, `طلب انضمام وكيل جديد: ${newReq.name} (${newReq.company || newReq.city})`);

            return sendJson(200, {
                success: true,
                message: 'تم إرسال طلب انضمامك كوكيل معتمد بنجاح! طلبك قيد مراجعة الإدارة العليا وسيتم تفعيل حسابك والتواصل معك قريباً. 🎉'
            });
        }

        // ==========================================
        // 👑 ADMIN: MANAGE AGENT APPLICATIONS
        // ==========================================

        // Get Pending Agent Requests
        if (pathname === '/admin/agent-requests' && method === 'GET') {
            if (!checkAdminAuth()) return;
            const requests = getAgentRequests();
            const pending = requests.filter(r => r.status === 'pending');
            return sendJson(200, { success: true, requests: pending });
        }

        // Approve Agent Application
        if (pathname === '/admin/agent-requests/approve' && method === 'POST') {
            if (!checkAdminAuth()) return;
            const { requestId, credits = 10, commission = '30%' } = body;

            const requests = getAgentRequests();
            const reqIdx = requests.findIndex(r => r.id === requestId);
            if (reqIdx === -1) return sendJson(404, { success: false, error: 'الطلب غير موجود' });
            ...
            const newAgent = {
                ...
                passwordHash: targetReq.passwordHash,
                phone: targetReq.phone,
                company: targetReq.company || targetReq.city || 'وكالة معتمدة',
                credits: parseInt(credits) || 10,
                totalIssued: 0,
                commission: commission || '30%',
                status: 'active',
                notes: `تم الاعتماد من طلب الانضمام: ${targetReq.city || ''} - ${targetReq.notes || ''}`,
                createdAt: new Date().toISOString()
            };
            ...
            addAuditLog('✅ AGENT_APPROVED', 'ADMIN', `تم قبول واعتماد الوكيل: ${newAgent.name} برصيد ${newAgent.credits} ترخيص وعمولة ${newAgent.commission}`);

            return sendJson(200, {
                success: true,
                message: `تم اعتماد وقبول الوكيل (${newAgent.name}) بنجاح وتخصيص ${newAgent.credits} ترخيص له! 🎉`,
                agent: newAgent
            });
        }

        // Reject Agent Application
        if (pathname === '/admin/agent-requests/reject' && method === 'POST') {
            if (!checkAdminAuth()) return;
            ...
            targetReq.rejectReason = reason || 'لم يستوف الشروط المطلوبة';
            saveAgentRequests(requests);

            addAuditLog('❌ AGENT_REJECTED', 'ADMIN', `تم رفض طلب الوكيل: ${targetReq.name}`);
            return sendJson(200, { success: true, message: 'تم رفض طلب الانضمام.' });
        }

        // ==========================================
        // 👔 AGENT SELF-PORTAL & DASHBOARD
        // ==========================================
        ...
        if (pathname === '/agent/auth/login' && method === 'POST') {
            ...
            if (!agent) {
                ...
                const requests = getAgentRequests();
                const pendingReq = requests.find(r => r.username.toLowerCase() === cleanUser);
                if (pendingReq) {
                    if (pendingReq.status === 'pending') {
                        return sendJson(403, { success: false, error: 'طلب حسابك كوكيل قيد المراجعة والاعتماد من قبل الإدارة العليا ⏳' });
                    } else if (pendingReq.status === 'rejected') {
                        return sendJson(403, { success: false, error: 'عذراً، تم رفض طلب الانضمام كوكيل من قبل الإدارة.' });
                    }
                }
                return sendJson(401, { success: false, error: 'اسم مستخدم الوكيل أو كلمة المرور غير صحيحة' });
            }

            if (agent.passwordHash !== hashPassword((password || '').trim())) {
                return sendJson(401, { success: false, error: 'كلمة المرور غير صحيحة' });
            }

            if (agent.status === 'suspended') {
                return sendJson(403, { success: false, error: 'حساب الوكيل موقوف حالياً من قبل الإدارة 🛑' });
            }
            ...
            return sendJson(200, {
                ...
                agent: {
                    ...
                    commission: agent.commission || '30%'
                }
            });
        }

        // Agent My Dashboard Stats & Clients
        if (pathname === '/agent/my-dashboard' && method === 'GET') {
            ...
            if (!session || !session.isAgent || !session.agent) {
                return sendJson(401, { success: false, error: 'جلسة الوكيل غير مصرح بها أو منتهية' });
            }
            ...
            return sendJson(200, {
                ...
                stats: {
                    ...
                    remainingCredits: agent.credits || 0
                },
                clients: myClients.map(c => {
                    const lastSeenMs = c.lastSeenAt ? new Date(c.lastSeenAt).getTime() : 0;
                    ...
                    return {
                        ...
                        status: c.status,
                        isOnline,
                        whatsappStatus: c.whatsappStatus || 'disconnected',
        ...
        if (pathname === '/agent/client/create' && method === 'POST') {
            ...
            if ((agent.credits || 0) < 1) {
                return sendJson(400, { success: false, error: 'رصيد التراخيص لديك غير كافٍ! يرجى طلب شحن رصيد من الإدارة العليا' });
            }

            const { username, password, name, company, phone, plan = '1year' } = body;
            if (!username || !password || !name) {
                return sendJson(400, { success: false, error: 'يرجى ملء الحقول الإجبارية للعميل' });
            }

            const cleanUser = username.trim().toLowerCase();
            const users = getUsers();
            if (users.some(u => u.username.toLowerCase() === cleanUser)) {
                return sendJson(400, { success: false, error: 'اسم المستخدم هذا مسجل بالفعل لعميل آخر' });
            }
            ...
            const cleanHWID = 'DESKTOP-APP-AGENT-' + cleanUser.toUpperCase();
            ...
            agent.credits = Math.max(0, (agent.credits || 1) - 1);
            agent.totalIssued = (agent.totalIssued || 0) + 1;
            saveAgents(agents);

            addAuditLog('✨ AGENT_ISSUE', agent.username, `إصدار ترخيص (${plan}) للعميل: ${newClient.name} (${newClient.username}) - الرصيد المتبقي: ${agent.credits}`);

            return sendJson(200, {
                success: true,
                message: `تم إنشاء وتفعيل حساب العميل بنجاح! تم خصم ترخيص واحد. رصيدك المتبقي: ${agent.credits}`,
                client: {
        ...
        if (pathname === '/admin/agents/recharge' && method === 'POST') {
            ...
            addAuditLog('⚡ AGENT_RECHARGE', 'ADMIN', `شحن رصيد للوكيل ${agent.name}: +${amount} ترخيص (الرصيد: ${agent.credits})`);
        ...
        if (pathname === '/admin/send-user-message' && method === 'POST') {
            ...
            addAuditLog('💬 DIRECT_MSG', user.username, `إرسال تنبيه خاص: ${text.substring(0, 30)}...`);
        ...
        // Client User Profile (For Desktop App)
